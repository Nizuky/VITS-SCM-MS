    ...
    <div class="container max-w-md mx-auto p-6">
       <h2 class="mb-4" style="font-weight: bold; text-align: center; font-size: 20px;">Admin Login</h2>
        ...
        <form id="admin-login-form" method="POST" action="{{ route('admin.login.submit') }}">
            @csrf
            @if(session('success'))
                <div class="mb-3 p-3 rounded text-sm" style="background:#e6ffef; border-left:4px solid #24a148; color:#064e3b;">{{ session('success') }}</div>
            @endif
            @if(session('error'))
            ...
            <div class="mb-3 text-right">
                <a href="{{ route('admin.password.request') }}" class="text-sm underline">Forgot your password?</a>
            </div>
